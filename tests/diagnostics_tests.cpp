...
void record_leaks(std::size_t outstanding) noexcept {
    reported_leaks = outstanding;
}

void double_free_and_state_queries(TestContext& test) {
    memory_pool::FixedSizeMemoryPool pool(32, 2, alignof(std::max_align_t),
                                          diagnostic_options());
    void* block = pool.allocate();
    test.expect(pool.block_index(block) == 0, "the first allocation should use block zero");
    test.expect(pool.is_allocated(block), "an allocated block should be tracked");
    pool.validate_integrity();

    pool.deallocate(block);
    test.expect(!pool.is_allocated(block), "a returned block should be tracked as free");
    pool.validate_integrity();
...
void memory_poisoning(TestContext& test) {
    memory_pool::FixedSizeMemoryPool pool(32, 1, alignof(std::max_align_t),
                                          diagnostic_options(false, true));
    ...
    test.expect(allocated_pattern_present, "allocated memory should use the debug pattern");
...
void guard_corruption(TestContext& test) {
    {
        memory_pool::FixedSizeMemoryPool pool(16, 1, alignof(std::max_align_t),
                                              diagnostic_options(false, false, true));
        auto* block = static_cast<std::byte*>(pool.allocate());
    ...
    }
    {
        ...
        block[-1] = std::byte{0};
        test.expect_throws<memory_pool::MemoryCorruptionError>(
            [&] { pool.deallocate(block); }, "a front-canary change should be detected");
    }
}

void diagnostic_dump(TestContext& test) {
    memory_pool::FixedSizeMemoryPool pool(16, 2, alignof(std::max_align_t),
                                          diagnostic_options());
    void* block = pool.allocate();
...
void leak_callback(TestContext& test) {
    reported_leaks = 0;
    {
        ...
        memory_pool::FixedSizeMemoryPool pool(16, 2, alignof(std::max_align_t), options);
